...
TEST_CASE("shared_state stashes the escaped value without completing outer",
          "[callcc][shared_state]") {
    int value = 0;
    bool errored = false;
    bool stopped = false;
...
TEST_CASE("escape sender stops its local receiver and stashes the value",
          "[callcc][escape]") {
    int value = 0;
    bool errored = false;
    bool stopped = false;
...
TEST_CASE(
    "inner receiver injects the local stop token; forwards or substitutes",
    "[callcc][inner]") {
    int value = 0;
    bool errored = false;
    bool stopped = false;
...
TEST_CASE("call_cc escape completes outer with escaped value",
          "[callcc][escape]") {
...
TEST_CASE("call_cc forwards inner errors with identity", "[callcc][error]") {
    auto work = smd::call_cc<int>([](auto /*escape*/) {
        return ex::just(0) |
               ex::then([](int) -> int { throw std::runtime_error{"boom"}; });
    });
    bool errored = false;
    try {
        ...
        REQUIRE(std::string{e.what()} ==
                "boom"); // the inner error, not a stray one
    }
    REQUIRE(errored);
}

TEST_CASE("call_cc reports a throwing factory via set_error (no terminate)",
          "[callcc][error]") {
...
TEST_CASE("call_cc carries a non-exception_ptr inner error",
          "[callcc][error][sigs]") {
    auto work = smd::call_cc<int>([](auto /*escape*/) {
...
TEST_CASE("call_cc propagates upward cancellation to inner work",
          "[callcc][cancel]") {
    ...
    auto stoppable = ex::write_env(
        std::move(work), ex::prop{ex::get_stop_token, outer_src.get_token()});
...
TEST_CASE(
    "call_cc escape waits for the inner op to drain before completing outer",
    "[callcc][cancel][drain]") {
    manual_controller ctrl;

    auto work = smd::call_cc<int>(
        [&](auto escape) { return escape_then_wait_sender{escape, &ctrl}; });

    std::optional<int> out;
    bool errored = false;
    bool stopped = false;
    auto op =
        ex::connect(std::move(work), drain_receiver{&out, &errored, &stopped});
    ex::start(op);
    ...
    // Drain the inner op; only now does the outer complete, with the escaped
    // value.
    ctrl.release();
